...
bool RobotisController::initialize(const std::string robot_file_path, const std::string init_file_path)
{
  ...
  //timecheck.open("/home/robotis/catkin_ws/src/ROBOTIS-Framework/robotis_controller/src/time.txt");
  queue_thread_ = boost::thread(boost::bind(&RobotisController::msgQueueThread, this));
  return true;
}
      ...
      if (dxl->bulk_read_items_.size() != 0)
      {
        ...
        for (int i = 0; i < dxl->bulk_read_items_.size(); i++)
        {
          ...
          for (int l = 0; l < addr_leng; l++)
          {
            ...
            {
              ROS_INFO("write2Byte(%s, %d, %d);",joint_name.c_str(),indirect_addr, dxl->ctrl_table_[dxl->bulk_read_items_[i]->item_name_]->address_ + l);
...
void RobotisController::msgQueueThread()
{
  ...
  ros::Subscriber online_walking_param_sub= ros_node.subscribe("/robotis/online_param", 1,
                                                               &RobotisController::onlinewalkingparamCallback, this);                                                               
...
//      }
  ...
  for (auto& dxl_it : robot_->dxls_)
  {
    ...
    //printf(" joint name: %s, torque=%f\n",joint_name,dxl->dxl_state_->present_torque_);
...
void RobotisController::onlinewalkingparamCallback(const robotis_controller_msgs::OnlineWalkingParam::ConstPtr &msg)
{
  //ROS_INFO("receive walking param msg = %d",msg->msgtest);
}
